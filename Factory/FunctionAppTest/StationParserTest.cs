...
public class StationParserTest
{
    ...
    public void ShouldParseRecord(string input, string id, string lat, string lon, string elevation, string state, string name, string flags, string wmoId)
    {
        ...
        var expected = new Station(id, double.Parse(lat, CultureInfo.InvariantCulture), double.Parse(lon, CultureInfo.InvariantCulture), double.Parse(elevation, CultureInfo.InvariantCulture), state, name, emptyRemoved, wmoId);
        var result = StationParser.ParseRecord(input);

        Assert.AreEqual(expected.id, result.id);
        Assert.AreEqual(expected.lat, result.lat);
        Assert.AreEqual(expected.lon, result.lon);
